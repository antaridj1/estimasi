...
class HitungController extends Controller
{
    ...
    public function index(){
        ...
        $indeks = Indek::where([['parameter','klasifikasi'],['status','1']])->orderBy('bobot_indeks')->get();
        $fungsis = Indek::where([['parameter','fungsi'],['status','1']])->get();
        $jangka_waktu = Indek::where([['parameter','waktu'],['status','1']])->orderBy('bobot_indeks')->get();
        $collection = Sarana::where('status','1')->get();
        $unique = $collection->unique('kategori');
        $kategori_sarana = $unique->values()->all();
        $saranas = Sarana::where('status','1')->get();
